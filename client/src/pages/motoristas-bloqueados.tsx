...
import { Pencil, Trash2, CheckCircle, Users, Ban, Search } from "lucide-react";
...
export default function MotoristasBloqueados() {
  ...
  const unblockMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || "Erro ao desbloquear motorista");
      }
  ...
  const handleUnblock = (driverId: string) => {
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Total de Bloqueados</CardTitle>
            <Ban className="h-4 w-4 text-red-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{totalBloqueados}</div>
            <p className="text-xs text-muted-foreground">
              Motoristas bloqueados no momento
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Motoristas Bloqueados</CardTitle>
        </CardHeader>
        <CardContent>
          {/* Campo de busca */}
          <div className="mb-6">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              Nenhum motorista bloqueado encontrado.
            </div>
          ) : (
            <Table>
              <TableHeader>
